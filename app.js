...
function newQuestion() {
  ...
  const t = +(Math.random() * 8 + 3).toFixed(1);
  const answer = key === 'cyl' ? cylinderVolume(r, t) : coneVolume(r, t);
  current = { key, r, t, answer };
  ...
  setLabel(SOLIDS[key].label, [
    ...
    { text: 'Berapakah Volumenya?', size: 44, color: '#eab308', weight: '800' },
  ]);

  const name = key === 'cyl' ? 'tabung' : 'kerucut';
  quizPrompt.innerHTML = `Hitung volume dimensi model 3D di atas meja:<br><b>Jari-jari (r) = ${r} cm</b>, <b>Tinggi (t) = ${t} cm</b>`;
...
function checkAnswer() {
  ...
  const { key, r, t, answer } = current;
  const within = Math.abs(guess - answer) <= answer * 0.02 + 0.05;
  const formula = key === 'cyl' ? `V = π · r² · t = π · ${r}² · ${t}` : `V = ⅓ · π · r² · t = ⅓ · π · ${r}² · ${t}`;

  if (visible.has(key)) setDefaultLabel(key);

  quizFeedback.className = 'quiz-feedback ' + (within ? 'ok' : 'no');
  quizFeedback.innerHTML = within
    ? `🎉 Benar! <br><small>${formula} ≈ <b>${fmt(answer)}</b> cm³</small>`
    : `❌ Kurang tepat. <br><small>${formula} ≈ <b>${fmt(answer)}</b> cm³ (jawabanmu: ${fmt(guess)} cm³)</small>`;
