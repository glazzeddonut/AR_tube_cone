...
function defaultLabelLines(key) {
  ...
  return [
    ...
    { text: isCone ? 'L = π · r · (r + s)' : 'L = 2 · π · r · (r + t)', size: 44, color: '#F8FAFC' },
  ];
}
...
const state = { wire: false, quiz: false, current: null };
let lastSeen = 'cyl';
...
function runBreakdown() {
  ...
  at(DUR * 5.6, () => {
    ...
    setLabel(s.label, [
      ...
      { text: isCone ? 'L = π · r · (r + s)' : 'L = 2 · π · r · (r + t)', size: 48, color: '#22c55e', weight: '800' },
    ]);
  });
...
const quizTabVol = $('quiz-tab-vol');
const quizTabArea = $('quiz-tab-area');

// current: { key, r, t, stage, answer, answered: { vol: bool, area: bool } }
let current = null;
...
function renderQuizStageUI() {
  const isVol = current.stage === 'vol';
  quizTabVol.classList.toggle('active', isVol);
  quizTabArea.classList.toggle('active', !isVol);
  quizTabVol.classList.toggle('answered', current.answered.vol);
  quizTabArea.classList.toggle('answered', current.answered.area);
...
function newQuestion() {
  ...
  current = { key, r, t, stage: 'vol', answer: 0, answered: { vol: false, area: false } };
  goToStage('vol');
}

// Pindah tahap (Volume <-> Luas) untuk soal & dimensi yang sama, bisa kapan saja
function goToStage(stage) {
  if (!current) return;
  cancelBreakdown();
  const { key, r, t } = current;
  current.stage = stage;
  current.answer = quizAnswerFor(key, r, t, stage);
  ...
  quizPrompt.innerHTML = `${task} dimensi model 3D di atas meja:<br><b>Jari-jari (r) = ${r} cm</b>, <b>Tinggi (t) = ${t} cm</b>`;
...
quizTabVol.addEventListener('click', () => { if (current && current.stage !== 'vol') goToStage('vol'); });
quizTabArea.addEventListener('click', () => { if (current && current.stage !== 'area') goToStage('area'); });

quizNext.addEventListener('click', () => {
  if (!current) return;
  const otherStage = current.stage === 'vol' ? 'area' : 'vol';
  if (!current.answered[otherStage]) {
    // Tahap lain (untuk soal yang sama) belum dijawab -> tawarkan itu dulu
    goToStage(otherStage);
  } else {
    newQuestion();
  }
});
...
function checkAnswer() {
  ...
  const otherStage = stage === 'vol' ? 'area' : 'vol';
  const otherLabel = otherStage === 'vol' ? 'Volume' : 'Luas Permukaan';
  quizNext.textContent = current.answered[otherStage] ? 'Pertanyaan baru' : `Lanjut: ${otherLabel} →`;
